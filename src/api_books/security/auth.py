...
def create_refresh_token(claims: dict) -> str:
    ...
    now = datetime.now(tz=ZoneInfo('UTC'))
    time_plus = timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS)
    expire = now + time_plus
...
# Dependência para as rotas protegidas
...
def get_user_refreshed_tokenizer(db: DBService, token: str = Depends(oauth2_scheme)):
    credentials_exception = HTTPException(
        status_code=HTTPStatus.UNAUTHORIZED,
        detail='Could not validate credentials',
        headers={'WWW-Authenticate': 'Bearer'},
